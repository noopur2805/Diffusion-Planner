...
def dpm_sampler(
    ...
    with (nullcontext() if enable_grad else torch.no_grad()):
...
def shortcut_sampler(
    ...
    with (nullcontext() if enable_grad else torch.no_grad()):
        ...
        for i in range(n_steps):
            t_cur = ts[i].expand(B)
            x0_pred = dit(xt, t_cur, cross_c, route_lanes, neighbor_current_mask, d=d, scenario_id=scenario_id).reshape(B, P, -1, 4)
