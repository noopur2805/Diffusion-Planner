...
from diffusion_planner.model.diffusion_utils.sampling import dpm_sampler, shortcut_sampler
from diffusion_planner.model.diffusion_utils.sde import SDE, VPSDE_linear
from diffusion_planner.utils.normalizer import ObservationNormalizer, StateNormalizer
from diffusion_planner.model.module.mixer import MixerBlock
from diffusion_planner.model.module.dit import TimestepEmbedder, StepEmbedder, DiTBlock, FinalLayer
from diffusion_planner.conditioning import NUM_SCENARIO_TYPES
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.dit = DiT(
            ...
            use_shortcut=getattr(config, "use_shortcut", False),
            use_scenario_embed=getattr(config, "use_scenario_tag", False),
        )
        ...
        self._use_shortcut = getattr(config, "use_shortcut", False)
        self._shortcut_steps = int(getattr(config, "shortcut_steps", 1))
    ...
    def forward(self, encoder_outputs, inputs):
        ...
        if self.training:
            ...
            # Momentum-aware inference (Novelty 8) -- all opt-in via inputs dict;
            # when ``prev_plan_anchor`` is absent, ``warm_start_alpha == 0`` and
            # ``n_samples == 1`` this path is bit-identical to upstream.
            prev_plan_anchor = inputs.get("prev_plan_anchor", None)
            warm_start_alpha = float(inputs.get("warm_start_alpha", 0.0))
            n_samples = int(inputs.get("n_samples", 1))
            # GRPO opt-in: when set, the sampler keeps the autograd graph so
            # the policy gradient can flow from the loss back through the
            # denoising chain to the planner parameters.
            enable_grad_sampling = bool(inputs.get("enable_grad_sampling", False))

            if n_samples > 1:
                if self._guidance_fn is not None and not self._use_shortcut:
                    raise NotImplementedError(
                        "n_samples > 1 with classifier guidance is not supported; "
                        "use shortcut sampling or set n_samples=1."
                    )
                K = n_samples
                ego_neighbor_encoding = ego_neighbor_encoding.repeat_interleave(K, dim=0)
                route_lanes = route_lanes.repeat_interleave(K, dim=0)
                current_states = current_states.repeat_interleave(K, dim=0)
                neighbor_current_mask = neighbor_current_mask.repeat_interleave(K, dim=0)
                if scenario_id is not None:
                    scenario_id = scenario_id.repeat_interleave(K, dim=0)
                B = B * K

            noise = torch.randn(B, P, self._future_len, 4, device=current_states.device) * 0.5
            if prev_plan_anchor is not None and warm_start_alpha > 0.0:
                # Anchor arrives in physical units (current ego frame); map to
                # normalized space so it matches the diffusion target distribution.
                anchor_norm = self._state_normalizer(prev_plan_anchor).to(
                    device=noise.device, dtype=noise.dtype
                )
                if anchor_norm.shape[0] != B:
                    anchor_norm = anchor_norm.expand(B, -1, -1, -1)
                future_init = noise.clone()
                # Apply warm-start to the ego row only; non-ego rows of the anchor
                # are zero-filled by the buffer and after normalization would
                # carry -mean/std, which we explicitly avoid here.
                future_init[:, 0] = warm_start_alpha * anchor_norm[:, 0] + noise[:, 0]
            else:
                future_init = noise

            xT = torch.cat([current_states[:, :, None], future_init], dim=2).reshape(B, P, -1)
            ...
            if self._use_shortcut:
                x0 = shortcut_sampler(
                    ...
                    n_steps=self._shortcut_steps,
                    correcting_xt_fn=initial_state_constraint,
                    sde=self._sde,
                    enable_grad=enable_grad_sampling,
                    scenario_id=scenario_id,
                )
...
class DiT(nn.Module):
    def __init__(self, sde: SDE, route_encoder: nn.Module, depth, output_dim, hidden_dim=192, heads=6, dropout=0.1, mlp_ratio=4.0, model_type="x_start", use_shortcut=False, use_scenario_embed=False, num_scenario_types=NUM_SCENARIO_TYPES):
        super().__init__()

        assert model_type in ["score", "x_start"], f"Unknown model type: {model_type}"
        self._model_type = model_type
        self._use_shortcut = use_shortcut
        self._use_scenario_embed = use_scenario_embed
        ...
        self.d_embedder = StepEmbedder(hidden_dim) if use_shortcut else None
        self.scenario_embedder = nn.Embedding(num_scenario_types, hidden_dim) if use_scenario_embed else None
    ...
    def forward(self, x, t, cross_c, route_lanes, neighbor_current_mask, d=None, scenario_id=None):
        """
        Forward pass of DiT.
        x: (B, P, output_dim)   -> Embedded out of DiT
        t: (B,)
        cross_c: (B, N, D)      -> Cross-Attention context
        d: (B,) optional        -> shortcut step-size in [0,1]. Ignored unless use_shortcut.
        scenario_id: (B,) long optional -> nuPlan scenario-type id. Ignored unless use_scenario_embed.
        """
        B, P, _ = x.shape
        ...
        if self._use_shortcut and d is not None:
            y = y + self.d_embedder(d)
        if self._use_scenario_embed and self.scenario_embedder is not None and scenario_id is not None:
            y = y + self.scenario_embedder(scenario_id.to(y.device))
