...
class StepEmbedder(nn.Module):
    """
    Embeds the (continuous) shortcut step-size $d \\in [0, 1]$ used by the
    Shortcut Forcing world model (Frans et al., 2024) into a vector that is
    added to the diffusion-time embedding inside each DiT block.
    """
    def __init__(self, hidden_size, frequency_embedding_size=256):
        super().__init__()
        self.mlp = nn.Sequential(
    ...
    def forward(self, d):
        d_freq = TimestepEmbedder.timestep_embedding(d, self.frequency_embedding_size)
        return self.mlp(d_freq)
