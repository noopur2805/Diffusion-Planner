...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.use_camera = bool(getattr(config, "use_camera", False))
        self.num_cameras = int(getattr(config, "num_cameras", 8))
        if self.use_camera:
            self.camera_encoder = CameraEncoder(
                hidden_dim=config.hidden_dim,
                num_views=self.num_cameras,
                backbone=getattr(config, "camera_backbone", "resnet18"),
                pretrained=getattr(config, "camera_pretrained", True),
                dropout=config.encoder_drop_path_rate,
            )
            self.fusion_token_num = self.token_num + self.num_cameras
        else:
            self.camera_encoder = None
            self.fusion_token_num = self.token_num

        self.use_scenario_tag = bool(getattr(config, "use_scenario_tag", False))
    ...
    def forward(self, inputs):
        ...
        fused_mask = encoding_mask.view(B, self.token_num)

        if self.use_camera and 'camera_images' in inputs:
            cam_tokens, cam_mask = self.camera_encoder(
                inputs['camera_images'],
                mask=inputs.get('camera_mask', None),
            )
            encoding_input = torch.cat([encoding_input, cam_tokens], dim=1)
            fused_mask = torch.cat([fused_mask, cam_mask], dim=1)

        if self.use_scenario_tag and 'scenario_id' in inputs:
            encoder_outputs['scenario_id'] = inputs['scenario_id'].long()

        encoder_outputs['encoding'] = self.fusion(encoding_input, fused_mask)
