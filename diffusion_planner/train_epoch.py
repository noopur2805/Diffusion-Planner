...
from diffusion_planner.loss import diffusion_loss_func, shortcut_loss_func
...
def train_epoch(data_loader, model, optimizer, args, ema, aug: StatePerturbation=None):
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            # optional camera modality (NavSim 8-view)
            if getattr(args, "use_camera", False) and len(batch) >= 13:
                inputs['camera_images'] = batch[11].to(args.device)
                inputs['camera_mask'] = batch[12].to(args.device)
            ...
            if getattr(args, "use_shortcut", False):
                loss, _ = shortcut_loss_func(
                    model,
                    inputs,
                    ...
                    k_max=getattr(args, "shortcut_k_max", 16),
