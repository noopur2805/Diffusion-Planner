...
from diffusion_planner.utils.navsim_dataset import NavSimMultiModalData
...
def get_args():
    ...
    parser.add_argument('--normalization_file_path', default='configs/normalization.json', help='filepath of normalizaiton.json', type=str)
    ...
    # Early stopping (opt-in; 0 = disabled, behaviour identical to prior versions)
    parser.add_argument('--early_stop_patience', type=int, default=0,
                        help='Stop training if epoch loss does not improve for this many epochs (0 = disabled).')
    ...
    # Shortcut Forcing (DreamerAD)
    parser.add_argument('--use_shortcut', default=False, type=boolean, help='Train DiT with Shortcut Forcing for 1-step inference')
    parser.add_argument('--shortcut_k_max', type=int, default=16, help='Maximum number of steps in the shortcut dyadic grid')
    parser.add_argument('--shortcut_steps', type=int, default=1, help='Number of sampler steps at inference when use_shortcut is on')

    # NavSim 8-camera (DreamerAD)
    parser.add_argument('--use_camera', default=False, type=boolean, help='Fuse NavSim 8-camera images into the encoder')
    parser.add_argument('--num_cameras', type=int, default=8, help='Number of camera views (NavSim = 8)')
    parser.add_argument('--camera_backbone', type=str, default='resnet18', help='timm backbone for the camera encoder')
    parser.add_argument('--camera_pretrained', default=True, type=boolean)
    parser.add_argument('--camera_dir', type=str, default=None, help='Root folder containing <token>/cam_*.jpg')
    parser.add_argument('--camera_manifest', type=str, default=None, help='Optional JSON mapping token->{view: path}')
    parser.add_argument('--image_size_h', type=int, default=224)
    parser.add_argument('--image_size_w', type=int, default=480)

    # Scenario-tag conditioning (optional categorical conditioning on nuPlan scenario_type)
    parser.add_argument('--use_scenario_tag', default=False, type=boolean, help='Add ground-truth nuPlan scenario_type as a categorical conditioning signal to the DiT')
...
def model_training(args):
    ...
    if args.use_camera:
        train_set = NavSimMultiModalData(
            args.train_set, args.train_set_list,
            args.agent_num, args.predicted_neighbor_num, args.future_len,
            camera_dir=args.camera_dir, camera_manifest=args.camera_manifest,
    ...
    for epoch in range(init_epoch, train_epochs):
        ...
        if global_rank == 0:
            ...
            should_stop = early_stopper.step(float(train_total_loss), epoch + 1)
            if early_stopper.improved and not early_stopper.disabled:
                save_model(diffusion_planner, optimizer, scheduler, save_path, epoch, train_total_loss, wandb_logger.id, model_ema.ema, is_best=True)
                print(f"Best model (loss={train_total_loss:.4f}) saved in {save_path}\n")
            if should_stop:
                print(f"Early stopping at epoch {epoch+1}: no improvement for "
                      f"{early_stopper.bad_epochs} epochs (best={early_stopper.best:.4f} "
                      f"@ epoch {early_stopper.best_epoch}).")

        if args.ddp:
            stop_flag = torch.tensor(1 if early_stopper.should_stop else 0, device=args.device)
